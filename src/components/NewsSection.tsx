      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {news.map((item) => (
            <article
              key={item.id}
              className="bg-white border border-warhammer-purple/20 p-6 rounded-lg shadow-md transform transition-transform hover:-translate-y-2"
            >
              <div className="text-warhammer-gray text-sm mb-2">{item.date}</div>
              <h3 className="font-gothic text-xl text-warhammer-purple mb-3">{item.title}</h3>
              <p className="text-warhammer-gray">{item.excerpt}</p>
            </article>
          ))}
        </div>
      </div>
